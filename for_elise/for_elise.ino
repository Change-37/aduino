...
void loop() {
  if (millis() - mil > 100) {
    ...
    if(song_pivot == 35)  song_pivot = 0; // 마지막 음까지 울렸다면 처음으로 돌아가서 다시 노래함
    mil = millis();
  }
}
...
void sing(int eum, int Long) {
  ...
  if(Long == 3) lele = 375000;
  else if(Long == 6) lele = 750000;
  
  for(long i=0; i< lele ; i+= eum) { // TODO: 왜 이런 for문을 사용했는지 알아보기
